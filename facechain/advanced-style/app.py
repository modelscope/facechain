...
def train_lora_fn(foundation_model_path=None, revision=None, output_img_dir=None, work_dir=None):
    os.system(f'PYTHONPATH=. accelerate launch train_text_to_image_lora.py --pretrained_model_name_or_path={foundation_model_path} '
...
def launch_pipeline(uuid,
    ...
    gen_portrait = GenPortrait(style_model_path, multiplier_style, add_prompt, use_main_model, use_face_swap, use_post_process,
                               use_stylization)
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                gr.Examples(examples['paths'], inputs=[style_model], label='风格模型列表')
